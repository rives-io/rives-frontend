...
function TapeFeesManager({address}:{address:string}) {
    // state
    ...
    useEffect(() => {
        if (!tapeContract || !signer || !signerAddress || !erc20ContractAddress) {
            setBalace(undefined);
            return;
        }
        tapeContract.accounts(signerAddress, erc20ContractAddress).then((data:BigNumber) => {
            setBalace(data);
        });
        
    }, [tapeContract,signerAddress,erc20ContractAddress,reload])
    ...
    async function withdraw() {
        ...
        try{
            const tx = await tapeContract.withdrawBalance(erc20ContractAddress,balance);
...
export default TapeFeesManager;