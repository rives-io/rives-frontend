...
import { cartridgeInfo, createRule, rules, ruleTags as getRuleTags, formatInCard, deactivateRule } from "../backend-libs/core/lib";
import Rivemu, { RivemuRef } from "./Rivemu";
import { CartridgeInfo, RuleInfo, InfoCartridge, RuleDataProxy, FormatInCardPayload } from "../backend-libs/core/ifaces";
...
import { generateEntropy } from '@/app/utils/util';
...
const doomArgsFields = {
    ...
    "numlevelsquit": {
        "field":"Number of Maps",
        "format":"-numlevelsquit :value:"
    },
    "turbo": {
        "field":"Speed",
        "format":"-turbo :value:"
    ...
    "autoquit": {
        "field":"Quit on Endgame and Menu Open",
        "format":"-autoquit"
    },
    "nomenu": {
        "field":"Disable Menu",
    ...
    "nocheat": {
        "field":"Disable Cheats",
        "format":"-nocheat"
    },
};

const doomArgsSchemaJson = //zodToJsonSchema(doomArgsSchema);
{
    "type": "object",
    "properties": {
        "deathquit": {
    ...
    "required":["deathquit","autoquit","nomenu","nocheat"],
    "additionalProperties": false,
};

function RulesEditor({cartridge_id, rule_id}:{
    cartridge_id:string, rule_id?: string}) {
    // rivemu state
    ...
    const [detailedRuleArgs, setDetailedRuleArgs] = useState<Record<string,unknown>>({});
    const [ruleArgsSchema, setRuleArgsSchema] = useState<Record<string,unknown>>();
    ...
    const [ruleArgsErrors, setRuleArgsErrors] = useState<Map<string,string>>(new Map());
    ...
    useEffect(() => {
        if (cartridge_id) {
            ...
            if (cartridge_id == '721f735bbca3') {
                const schema = doomArgsSchemaJson;
                setRuleArgsSchema(schema);
                const newDetailedArgs: Record<string,any> = {};
                if (schema){
                    const properties = schema["properties"] as Record<string,Record<string,unknown>>;
                    for (const key in properties) {
                        const defValue = properties[key]["default"] || properties[key]["const"];
                ...
                const argsMap = new Map(Object.entries(doomArgsFields));
                updateArgs(argsMap,newDetailedArgs);
                setRuleArgsFields(argsMap);
                setRuleArgsFieldsKeys(Object.keys(doomArgsFields));
            }
        }
    }, [cartridge_id]);
    ...
    function updateArgs(argsFields: Map<string,unknown>,detailedArgs: Record<string,unknown>) {
            ...
            if (!detailValid) {
                ...
                if (validator.errors)
                    for (const err of validator.errors) {
                        newErrors.set(err['instancePath'].replace('/',''),err['message'] as string)
                    }
    ...
    const rivemuOnBegin = function (width: number, height: number, target_fps: number, total_frames: number, info_data: Uint8Array) {
        ...
        if (info_data.length > 0) {
            try {
                ...
                if (info.fields && info.schema) {

                    const schema = info.schema;
                    setRuleArgsSchema(schema);
                    const newDetailedArgs: Record<string,any> = {};
                    if (schema){
                        const properties = schema.get("properties") as Record<string,Record<string,unknown>>;
                        for (const key in properties) {
                            const defValue = properties[key]["default"] || properties[key]["const"];
                            if (defValue != undefined) newDetailedArgs[key] = defValue;
                            else newDetailedArgs[key] = '';
        ...
        <div className="w-full">
            <div className='flex items-start flex-wrap justify-center gap-2'>
                <div className="grid grid-cols-1 place-items-center">
                    ...
                    {outcardObj && outcardObjKeys ? <div className="grid grid-cols-1 w-full text-sm justify-items-center">
                        ...
                        <tbody>
                            {outcardObjKeys.map((key,index) => {
                                ...
                                <tr key={"outcard"+index}>
                                    <td key={"outcard-k" + index} className='text-left'>{key}</td>
                                    <td key={"outcard-v" + index} className='text-right'>{outcardObj.get(key) as string}</td>
                                </tr>);
                            })}
                        </tbody>
                    </table>
                    </div> : <TextField className="w-full" label="Outcard" disabled value={outcard || ""} multiline variant="standard"  />}
                ...
                <div className="grid grid-cols-1 gap-4 place-items-left text-white xs:w-3/4 md:w-3/4 lg:w-1/3 xl:w-1/4 2xl:w-1/4">
                    <div className='grid grid-cols-1 gap-4 border border-stone-500 p-4'>
                        <TextField className="w-full" label="Name" value={ruleName || ""} variant="standard" 
                            disabled={rule != undefined} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setRuleName(e.target.value)} />
                        <TextField className="w-full" label="Description" value={ruleDescription || ""} multiline variant="standard" 
                            disabled={rule != undefined} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setRuleDescription(e.target.value)} />
                        <div ><LocalizationProvider dateAdapter={AdapterDayjs}>
                            <DateTimePicker label="Start (local)" value={ruleStart || null} disabled={rule != undefined} onChange={(newValue: Dayjs | null) => setRuleStart(newValue||undefined)} />
                        </LocalizationProvider></div>

                        <div ><LocalizationProvider dateAdapter={AdapterDayjs}>
                            <DateTimePicker label="End (local)" value={ruleEnd || null} disabled={rule != undefined} onChange={(newValue: Dayjs | null) => setRuleEnd(newValue||undefined)} defaultValue={null} />
                        </LocalizationProvider></div>

                        <TextField className="w-full" label="Score Function" value={ruleScoreFunction || ""} variant="standard" 
                            disabled={rule != undefined} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setRuleScoreFunction(e.target.value)} />

                        { !rule && ruleArgsFieldsKeys ? 
                            ruleArgsFieldsKeys.map((key,index) => {
                                if (!ruleArgsFields || !ruleArgsSchema) return null;
                                const fields = ruleArgsFields.get(key) as Record<string,string>;
                                const properties = ruleArgsSchema["properties"] as Record<string,Record<string,string>>;
                                const schema = properties[key];
                                if (!schema) return null;
                                // .get(key) as Record<string,string>
                                if (schema["type"] == 'boolean') {
                                    return (<FormControlLabel key={"arg-"+key+index} control={
                                        <Checkbox key={"argcheck-"+key+index} disabled={schema["const"] != undefined} 
                                            indeterminate={detailedRuleArgs[key] === ""} 
                                            checked={detailedRuleArgs[key] as boolean} onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChangeDetailedRuleArgs(key,e.target.checked)} />
                                        } label={fields["field"]} />);
                                }

                                return (
                                    <TextField key={"arginput-"+key+index} disabled={schema["const"] != undefined} className="w-full" label={fields["field"]} value={detailedRuleArgs[key]} variant="standard" 
                                    type={schema["type"]} onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChangeDetailedRuleArgs(key,e.target.value)} 
                                    error={ruleArgsErrors.get(key) != undefined} helperText={ruleArgsErrors.get(key)} />
                                );
                            })
                        : <></>}

                        <TextField className="w-full" label="Args" value={ruleArgs || ""} variant="standard" 
                            disabled={rule != undefined || ruleArgsFields != undefined} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setRuleArgs(e.target.value)} />

                        <FormControl variant="standard" >
                            <InputLabel htmlFor="incard">Incard (Hash) <UploadIcon onClick={uploadIncard} className='cursor-pointer'/></InputLabel>
                        ...
                        <input disabled={rule != undefined} type="file" ref={incardFileRef} onChange={(e) => handleOnChangeIncardUpload(e)} style={{ display: 'none' }}/>
                        ...
                        <Autocomplete
                            ...
                            onChange={(event: any, newValue: string[] | null) => setRuleTags(newValue||[])}
                        ...
                        {!rule ? <div className='grid grid-cols-1 gap-1 justify-items-center'>
                            ...
                            <button disabled={!ruleName || !ready || !user || !tested || !valid || argsErrors } title={sendRuleTooltip()} className="btn mt-2 text-xs shadow" onClick={sendRule} >
                                Create Play Mode
                            </button>

                            {/* <button disabled={!rule || !ready || !user} className="btn mt-2 text-[10px] shadow" onClick={sendRuleDeactivation} >
