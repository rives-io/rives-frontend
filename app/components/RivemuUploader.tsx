...
import { CartridgeInfo, InfoCartridge, InsertCartridgePayloadProxy } from "../backend-libs/core/ifaces";
...
function RivemuUploader() {
    ...
    useEffect(() => {
        ...
        if (!user) {
            ...
            setWallet(wallets.find((wallet) => wallet.address === user!.wallet!.address));
        }
    },[user]);
            ...
            <div className="flex items-center flex-wrap justify-center h-[calc(100vh-13rem)] gap-2 overflow-auto absolute top-16 w-full">
                <div className="grid grid-cols-1 place-items-center">
                    ...
                    >
                        ...
                        <div hidden={!cartridgeData}  className={'absolute t-0 relative'}>
                            ...
                            : (paused ?     
                                <button className={'absolute gameplay-screen-sm text-gray-500 hover:text-whte t-0 backdrop-blur-sm border border-gray-500'} onClick={pause}>
                                    <PlayArrowIcon className='text-7xl' />
                                </button>
                            : <></>)
                            }
                            <Rivemu ref={rivemuRef} cartridge_data={cartridgeData} args={args} entropy={entropy}
                                tape={tape}
                    ...
                    <TextField className="w-full" label="Outcard" multiline disabled value={outcard || ""} variant="standard" hidden={hideCartridgeParams}
                    ...
                    <TextField className="w-full" label="Rule Args" value={args || ""} variant="standard" hidden={hideCartridgeParams}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setArgs(e.target.value)} />

                    <FormControl className="w-full" variant="standard" hidden={hideCartridgeParams}>
                ...
                <div className="grid grid-cols-1 gap-4 place-items-left text-white xs:w-3/4 md:w-3/4 lg:w-1/3 xl:w-1/4 2xl:w-1/4">
                    ...
                    <div className='grid grid-cols-1 gap-4 border border-stone-500 p-4'>
                        ...
                        <TextField className="" label="Summary" disabled value={infoCartridge?.summary || ""} variant="standard"
                                multiline InputLabelProps={{ shrink: true }} />
                        <TextField className="" label="Description" disabled value={infoCartridge?.description || ""} variant="standard"
                                multiline InputLabelProps={{ shrink: true }} />
                        <TextField className="" label="Authors" disabled value={infoCartridge?.authors?.map((a,i) => a.name + (a.link ? `: ${a.link}` : '')).join("\n") || ""} variant="standard"
                                multiline InputLabelProps={{ shrink: true }} />
