...
import { CartridgeInfo, GetRulesPayload, RuleInfo } from "@/app/backend-libs/core/ifaces";
...
const getRules = async (cartridgeId: string, onlyActive = false) => {
  ...
  const allRules:Array<RuleInfo> = (await rules(inputPayload, {cartesiNodeUrl: envClient.CARTESI_NODE_URL, decode: true})).data
  return allRules;
}

export default async function PlayModes({ params }: { params: { cartridge_id: string } }) {
  const contests = (await getRules(params.cartridge_id)).sort((a, b) => {
      ...
      <section className="flex flex-wrap justify-center gap-4">
          {
            contests.map((contest, index) => {
              ...
              return (
                <ContestCard key={index} contest={contest} activateNonContest={true}
                cartridge={{...cartridgeInfoMap[contest.cartridge_id], user: cartridgeCreatorUser}} />
              )
            })
          }
          <NewContestCard cartridgeId={params.cartridge_id} />
      </section>
    </main>
  )
}
