...
export async function generateMetadata({ params }: { params: { rule_id: string } }) {
    const cartridge_id = cartridgeIdFromBytes(params.rule_id);

    const rulePromise = getRuleInfo(params.rule_id);
        ...
        return (
            <main className="flex items-center justify-center h-lvh">
                <div className='flex w-96 flex-wrap break-all justify-center'>
                    ...
                    <span style={{color: 'white'}}> Rule {params.rule_id} not found!</span>
                </div>
            </main>
        )
    }
    ...
    return (
        <main>
            <section className='flex flex-col items-center gap-4'>
                <RivemuPlayer rule_id={params.rule_id}/>
                {
                    [ContestStatus.IN_PROGRESS,ContestStatus.INVALID].indexOf(status) > -1?
                        <GameplaySubmitter />
                    :
                        <></>
                }
